...
## Purpose
Define the catalog that drives AI schema extraction and prompt generation for supported video node types.
...
- **WHEN** `defineCatalog()` is called with entries for `group`, `rect`, `text`, `math`, `functionGraph`, `parametricGraph`, `center`, `stack`, and `align`
...
The system SHALL provide a method to extract the combined Zod schema from the catalog for use with AI SDK structured output. The extracted schema SHALL be the AI-facing output schema used by the generation pipeline and SHALL include all node types declared in the catalog.
