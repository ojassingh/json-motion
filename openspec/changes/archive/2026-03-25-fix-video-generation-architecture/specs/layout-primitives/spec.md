...
Layout nodes SHALL support the same base `animate` and `primitives` properties as other nodes (opacity, rotate, scale, etc.). Animations on a layout node SHALL affect all of its children as a group (via the canvas transform stack). Layout-computed positions SHALL be calculated at the base (non-animated) state.
